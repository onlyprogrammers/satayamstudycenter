...
import { getNodePathFromEvent, getTextContent, isDeadClick, getElementSize, getUniqueSelector, isRageClick } from './zipy-utils.js';
    ...
    const getHandler = (eventKey) => {
        return (event) => {
            ...
            if (MouseInteractions[eventKey] === MouseInteractions.Click) {
                const screenHeight = window.innerHeight ? window.innerHeight : null;
                const screenWidth = window.innerWidth ? window.innerWidth : null;
                const navigationUrl = window.location.href || '';
                const innerText = getTextContent(event);
                const deadClick = isDeadClick(event);
                const offsetX = event.offsetX;
                const offsetY = event.offsetY;
                const [elementHeight, elementWidth] = getElementSize(event);
                const selector = getUniqueSelector(event);
                const currentTime = new Date().getTime();
                let rageClick = false;
                if (lastSelector === selector) {
                    rageClick = isRageClick(currentTime, lastEventTime);
                }
                lastEventTime = new Date(currentTime).getTime();
                lastSelector = selector || '';
                mouseEvent = Object.assign(Object.assign({}, mouseEvent), { deadClick,
                    innerText,
                    offsetX,
                    offsetY,
                    rageClick,
            ...
            callbackWrapper(mouseInteractionCb)(mouseEvent);
        };
    };
